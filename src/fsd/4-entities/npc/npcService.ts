...
export class NpcService {
  ...
  public static getNpcIconPath(name: string): string {
    ...
    const map: Record<string, string> = {
      ...
      "Skitarii Vanguard": "skitarii_vanguard.webp",
      "Electro-Priest": "electro-priest.webp",
      "Tech-Priest Enginseer": "tech-priest_enginseer.webp",
    };
    if (map[name]) {
  ...
  private static convertNpcData(): INpcData[] {
    ...
    data = npcData.npcs.map((npc) => {
      const faction: Faction =
        parseFaction(npc.faction) ?? Faction.Ultramarines;
      const alliance: Alliance =
        this.parseAlliance(npc.alliance) ?? Alliance.Imperial;
      const ret: INpcData = {
        name: npc.name,
        faction: faction,
        alliance: alliance,
